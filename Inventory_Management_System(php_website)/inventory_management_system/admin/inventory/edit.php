...
// Fetch item data
$stmt = $conn->prepare('SELECT name, category, quantity, unit_price, supplier_id FROM Inventory WHERE id = ?');
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($name, $category, $quantity, $unit_price, $supplier_id);
if (!$stmt->fetch()) {
    $stmt->close();
    header('Location: index.php');
    exit;
...
<div class="container mt-5">
  ...
  <form method="post" action="">
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control" id="quantity" name="quantity" min="0" value="<?php echo $quantity; ?>" required>
    </div>
    <div class="mb-3">
      <label for="unit_price" class="form-label">Unit Price</label>
      <input type="number" step="0.01" class="form-control" id="unit_price" name="unit_price" min="0" value="<?php echo $unit_price; ?>" required>
    </div>
    <div class="mb-3">
      ...
      <select class="form-select" id="supplier_id" name="supplier_id">
        ...
        <?php while ($row = $suppliers->fetch_assoc()): ?>
          <option value="<?php echo $row['id']; ?>" <?php if ($row['id'] == $supplier_id) echo 'selected'; ?>><?php echo htmlspecialchars($row['name']); ?></option>
        <?php endwhile; ?>
      </select>
    </div>
