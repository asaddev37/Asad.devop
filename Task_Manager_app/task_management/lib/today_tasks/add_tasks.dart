...
class _AddTasksPageState extends State<AddTasksPage> {
  String title = '';
  String description = '';
  DateTime dueDate = DateTime.now().add(const Duration(minutes: 5));
  ...
  Future<void> _loadDraftTask() async {
    ...
    final draftTaskJson = prefs.getString('draft_task');
  ...
  Future<void> _saveDraftTask() async {
    ...
    await prefs.setString('draft_task', jsonEncode(draftTask));
  }

  Future<void> _clearDraftTask() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('draft_task');
  }
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        if (dueDate.isBefore(DateTime.now())) {
                          _showSnackBar('Please select a future due date!', isError: true);
                          return;
                        }
                        ...
                        if (confirmSave == true) {
                          ...
                          try {
                            ...
                            if (id != null && id > 0) {
                              final scheduledTask = Map<String, dynamic>.from(task);
                              scheduledTask['id'] = id;
                              if (widget.notificationsEnabled) {
                                await _scheduleNotification(scheduledTask);
                              }
                              ...
                              bool? returnToToday = await showDialog<bool>(
                                context: context,
                                builder: (context) => AlertDialog(
                                  backgroundColor: widget.isDarkMode ? Colors.grey[900] : Colors.white,
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                                  ...
                                  content: Text(
                                    'Task added successfully. Would you like to return to Today\'s Tasks?',
                                    style: TextStyle(
                                      color: widget.isDarkMode ? Colors.white70 : Colors.black87,
                                      fontSize: 16,
                                    ),
                              ...
                              _showSnackBar('Failed to add task: Invalid or no ID returned ($id)', isError: true);
                            }
                          } catch (e) {
