...
class IndividualSemesterGpaHistory extends StatefulWidget {
  final bool isDarkMode;
  const IndividualSemesterGpaHistory({super.key, required this.isDarkMode});
  @override
  State<IndividualSemesterGpaHistory> createState() => _IndividualSemesterGpaHistoryState();
}

class _IndividualSemesterGpaHistoryState extends State<IndividualSemesterGpaHistory> {
  ...
  Widget build(BuildContext context) {
    ...
    final hoverColor = widget.isDarkMode ? Colors.amberAccent.withAlpha(51) : Color(0xFFF39C12).withAlpha(51);
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Individual Semester History', style: TextStyle(color: Colors.white, fontSize: 18)),
                    ...
                    sliver: SliverList(
                      delegate: SliverChildBuilderDelegate(
                          ...
                          if (data.isEmpty) {
                            if (index == 0) {
                              return Center(
                                child: Padding(
                                  ...
                                  child: Text(
                                    'No history available yet.\nStart calculating to build your records!',
                            ...
                            final cgpa = record['cgpa'] as double? ?? 0.0;
                            ...
                            return Card(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Text('Semester: ${record['semester'] ?? 'N/A'}',
                                    ...
                                    SizedBox(height: 12),
                                    Text('Weighted GPA: ${cgpa.toStringAsFixed(2)}',
                                        style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: primaryTextColor)),
                                    SizedBox(height: 8),
  ...
  Future<List<Map<String, dynamic>>> _getIndividualHistory() async {
    final db = await DatabaseService.database;
    return await db.query('individual_semester_history');
  }
  ...
  Future<String> _saveCsvFile(List<Map<String, dynamic>> data) async {
    List<List<dynamic>> csvData = [
      ['Semester', 'Subjects', 'Credits', 'Grades', 'Weighted GPA', 'Timestamp']
    ];
    ...
    for (var record in data) {
      ...
      csvData.add([
        ...
        record['credits'] ?? '',
        record['grades'] ?? '',
        record['cgpa']?.toStringAsFixed(2) ?? '0.0',
        formattedTimestamp,
      ]);
    }
    ...
    String fileName = 'Individual_Semester_History_${DateTime.now().millisecondsSinceEpoch}.csv';
  ...
  Future<String> _savePdfFile(List<Map<String, dynamic>> data) async {
      ...
      pdf.addPage(
        pw.Page(
          build: (pw.Context context) => pw.Column(
            ...
            children: [
              pw.Text('Semester: ${record['semester'] ?? 'N/A'}',
              ...
              pw.SizedBox(height: 12),
              pw.Text('Weighted GPA: ${cgpa.toStringAsFixed(2)}',
                  style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: primaryTextColor)),
    ...
    String fileName = 'Individual_Semester_History_${DateTime.now().millisecondsSinceEpoch}.pdf';
    String filePath = '${directory.path}/$fileName';
    final file = File(filePath);
