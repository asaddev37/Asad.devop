...
import 'table_multiplication.dart';
import '../main.dart';

class MultiplicationBalloonPopGameModal extends StatefulWidget {
  const MultiplicationBalloonPopGameModal({super.key});

  @override
  _MultiplicationBalloonPopGameModalState createState() => _MultiplicationBalloonPopGameModalState();
}

class _MultiplicationBalloonPopGameModalState extends State<MultiplicationBalloonPopGameModal> with TickerProviderStateMixin {
  ...
  Future<void> _initializeAudioPlayers() async {
    try {
      ...
      try {
        ...
        debugPrint('🔊 Background music initialized in MultiplicationBalloonPopGameModal');
      } catch (e) {
        debugPrint('⚠️ Background music not available: $e');
      }
      debugPrint('🔊 AudioPlayers initialized in MultiplicationBalloonPopGameModal');
    } catch (e) {
  ...
  void _generateQuestion() {
    final maxFactor = (9 * difficulty).clamp(1, 9).toInt();
    final factor1 = random.nextInt(maxFactor) + 1;
    final factor2 = random.nextInt(maxFactor) + 1;
    final product = factor1 * factor2;
    if (mounted) {
      setState(() {
    ...
    if (!isGameOver && mounted) {
      setState(() {
        if (balloon.isStar) {
          ...
          _confettiController.play();
          final messages = ['Multiplication Master!', 'Math Superstar!', 'Awesome Job!', 'You Rock!'];
            ...
            AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  Text(
                    "You're a Multiplication Champ! Play again?",
                    style: TextStyle(
                      fontSize: 18,
                      color: isDarkMode ? Colors.white70 : Colors.black87,
                      fontFamily: 'Comic Sans MS',
  ...
  void dispose() {
    ...
    gameTimer?.cancel();
    messageTimer?.cancel();
    debugPrint('🗑️ Disposed AudioPlayers, ConfettiController, and Timers in MultiplicationBalloonPopGameModal');
    super.dispose();
  }
