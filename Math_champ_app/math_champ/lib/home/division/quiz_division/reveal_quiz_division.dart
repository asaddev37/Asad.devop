...
import '/main.dart';
import '../../addition/quiz_addition/quiz_game.dart';
import '../../table_multiplication/widgets.dart';
import 'choose_quiz_division.dart';

class RevealQuizDivision extends StatefulWidget {
  ...
  @override
  _RevealQuizDivisionState createState() => _RevealQuizDivisionState();
}

class _RevealQuizDivisionState extends State<RevealQuizDivision> with TickerProviderStateMixin {
  ...
  String _mascotMessage = "Time to divide, champ! 🚀";
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in RevealQuizDivision');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in RevealQuizDivision: $e');
    }
  }
  ...
  void _generateQuiz() {
    ...
    for (int divisor = widget.endNumber; divisor <= widget.startNumber; divisor++) {
      if (divisor == 0) continue;
      for (int quotient = 1; quotient <= widget.startNumber ~/ divisor; quotient++) {
        int dividend = divisor * quotient;
        if (dividend <= widget.startNumber) {
          if (widget.difficulty == Difficulty.easy ||
            ...
            possibleQuestions.add({
              ...
              'weight': widget.difficulty == Difficulty.hard
                  ? quotient * (divisor / widget.startNumber)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      showErrorOverlay(
        context: context,
        message: "Your division quiz adventure starts now! 🚀",
        onDismiss: () => _playSound('assets/sounds/cliick.wav'),
        tickerProvider: this,
      );
    });
  ...
  void _showQuizResults(int correct) {
    ...
    String message = correct >= widget.questionCount * 0.8
        ? "Wow, you’re a division superstar! Keep shining! 🌟"
          ...
          actions: [
            buildButton(
              ...
              onTap: () {
                _playSound('assets/sounds/cliick.wav');
                Navigator.pop(context);
                _showQuizResults(_userAnswers.where((answer) => answer != null).length);
              },
              isDarkMode: isDarkMode,
              themeProvider: themeProvider,
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
  ...
  void dispose() {
    ...
    debugPrint('🗑️ Disposed AudioPlayer and ConfettiController in RevealQuizDivision');
    super.dispose();
  }
}