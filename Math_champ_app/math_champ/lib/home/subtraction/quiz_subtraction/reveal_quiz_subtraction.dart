...
import 'set_quiz_subtraction.dart';

class RevealQuizSubtraction extends StatefulWidget {
  ...
  @override
  _RevealQuizSubtractionState createState() => _RevealQuizSubtractionState();
}

class _RevealQuizSubtractionState extends State<RevealQuizSubtraction> with TickerProviderStateMixin {
  ...
  String _mascotMessage = "Time to subtract, champ! 🚀";
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in RevealQuizSubtraction');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in RevealQuizSubtraction: $e');
    }
  }
  ...
  void _generateQuiz() {
      ...
      for (int subtrahend = widget.endNumber; subtrahend <= minuend; subtrahend++) {
        ...
        if (difference >= widget.endNumber && difference <= widget.startNumber) {
          if (widget.difficulty == Difficulty.easy ||
              (widget.difficulty == Difficulty.medium && difference >= 10) ||
              (widget.difficulty == Difficulty.hard && difference >= 20)) {
            possibleQuestions.add({
              ...
              'weight': widget.difficulty == Difficulty.hard
                  ? difference * (minuend / widget.startNumber)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      showErrorOverlay(
        context: context,
        message: "Your subtraction quiz adventure starts now! 🚀",
        onDismiss: () => _playSound('assets/sounds/cliick.wav'),
        tickerProvider: this,
      );
    });
  ...
  void _showQuizResults(int correct) {
    ...
    String message = correct >= widget.questionCount * 0.8
        ? "Wow, you’re a subtraction superstar! Keep shining! 🌟"
  ...
  void dispose() {
    ...
    debugPrint('🗑️ Disposed AudioPlayer and ConfettiController in RevealQuizSubtraction');
    super.dispose();
  }
}