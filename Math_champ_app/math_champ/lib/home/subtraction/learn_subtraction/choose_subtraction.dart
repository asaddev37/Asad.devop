...
import '/main.dart';
import 'reveal_subtraction.dart';

enum Difficulty { easy, medium, hard }

class ChooseSubtraction extends StatefulWidget {
  const ChooseSubtraction({super.key});

  @override
  _ChooseSubtractionState createState() => _ChooseSubtractionState();
}

class _ChooseSubtractionState extends State<ChooseSubtraction> with TickerProviderStateMixin {
  late AudioPlayer _audioPlayer;
  late ConfettiController _confettiController;
  int _largestNumber = 10;
  int _smallestNumber = 0;
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in ChooseSubtraction');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in ChooseSubtraction: $e');
    }
  }
  ...
  void _updateMaxQuestions() {
    int count = 0;
    for (int minuend = _smallestNumber + 1; minuend <= _largestNumber; minuend++) {
      for (int subtrahend = _smallestNumber; subtrahend < minuend; subtrahend++) {
        int difference = minuend - subtrahend;
        if (difference >= _smallestNumber) {
  ...
  void _updateNumberRanges() {
    setState(() {
      if (_difficulty == Difficulty.easy) {
        ...
        _smallestNumber = _smallestNumber.clamp(0, 9);
      } else if (_difficulty == Difficulty.medium) {
        _largestNumber = _largestNumber < 10 ? 10 : _largestNumber.clamp(10, 50);
        _smallestNumber = _smallestNumber < 1 ? 1 : _smallestNumber.clamp(1, 49);
      } else {
        _largestNumber = _largestNumber < 20 ? 20 : _largestNumber.clamp(20, 100);
        _smallestNumber = _smallestNumber < 2 ? 2 : _smallestNumber.clamp(2, 99);
      }
  ...
  Future<bool> _onWillPop() async {
    ...
    return await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to leave without starting your subtraction adventure? 🌟'),
        actions: [
          TextButton(
  ...
  void _validateAndProceed() {
    ...
    if (_smallestNumber < 0) {
      ...
      showErrorOverlay(
        context: context,
        message: 'The smallest number must be at least 0! 🌟',
        onDismiss: () => _playSound('assets/sounds/cliick.wav'),
        tickerProvider: this,
      );
      return;
    ...
    Navigator.push(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (context, animation, secondaryAnimation) => RevealSubtraction(
  ...
  void dispose() {
    ...
    debugPrint('🗑️ Disposed AudioPlayer and ConfettiController in ChooseSubtraction');
    super.dispose();
  }
                            ...
                            child: Container(
                              ...
                              child: Text(
                                _difficulty == Difficulty.easy
                                    ? "Let’s subtract easy, champ!"
                                    : _difficulty == Difficulty.medium
                                    ? "Ready for more subtracting?"
                                    : "Big subtraction, superstar!",
                                style: TextStyle(
                                  fontSize: 14,
