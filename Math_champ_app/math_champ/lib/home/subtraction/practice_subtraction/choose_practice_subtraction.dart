...
import '/main.dart';
import 'reveal_practice_subtraction.dart';

enum Difficulty { easy, medium, hard }

class ChoosePracticeSubtraction extends StatefulWidget {
  const ChoosePracticeSubtraction({super.key});

  @override
  _ChoosePracticeSubtractionState createState() => _ChoosePracticeSubtractionState();
}

class _ChoosePracticeSubtractionState extends State<ChoosePracticeSubtraction> with TickerProviderStateMixin {
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in ChoosePracticeSubtraction');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in ChoosePracticeSubtraction: $e');
    }
  }
  ...
  void _updateMaxQuestions() {
    int count = 0;
    for (int minuend = _smallestNumber; minuend <= _largestNumber; minuend++) {
      for (int subtrahend = _smallestNumber; subtrahend <= minuend; subtrahend++) {
        int difference = minuend - subtrahend;
        if (difference >= _smallestNumber && difference <= _largestNumber) {
          if (_difficulty == Difficulty.easy ||
              (_difficulty == Difficulty.medium && difference >= 5) ||
              (_difficulty == Difficulty.hard && difference >= 10)) {
            count++;
          }
        }
      }
  ...
  Future<bool> _onWillPop() async {
    ...
    return await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to leave without starting your subtraction practice? 🌟'),
        actions: [
          TextButton(
  ...
  void _validateAndProceed() {
    ...
    if (_questionCount > _maxQuestions) {
      ...
      showErrorOverlay(
        ...
        onDismiss: () => _playSound('assets/sounds/submit.wav'),
        tickerProvider: this,
      );
      return;
    }
    ...
    Navigator.push(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (context, animation, secondaryAnimation) => RevealPracticeSubtraction(
  ...
  void dispose() {
    ...
    debugPrint('🗑️ Disposed AudioPlayer and ConfettiController in ChoosePracticeSubtraction');
    super.dispose();
  }
                          ...
                          child: Container(
                            ...
                            child: Text(
                              _difficulty == Difficulty.easy
                                  ? "Let’s practice easy subtraction!"
                                  : _difficulty == Difficulty.medium
                                  ? "Ready for a subtraction challenge?"
                                  : "Tough subtraction, superstar! 🚀",
                              style: TextStyle(
                                fontSize: 14,
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: 12),
                            Text(
                              "Pick the largest and smallest numbers, then how many questions you want to practice! Bigger ranges = more fun! 🚀",
                              style: TextStyle(
                                fontSize: 16,
