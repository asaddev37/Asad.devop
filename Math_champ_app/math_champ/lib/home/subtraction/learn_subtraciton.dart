...
import '/main.dart';
import 'learn_subtraction/choose_subtraction.dart';


class LearnSubtraction extends StatefulWidget {
  const LearnSubtraction({super.key});

  @override
  _LearnSubtractionState createState() => _LearnSubtractionState();
}

class _LearnSubtractionState extends State<LearnSubtraction> with AutomaticKeepAliveClientMixin, SingleTickerProviderStateMixin {
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in LearnSubtraction');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in LearnSubtraction: $e');
    }
  }
        ...
        ..initialize().then((_) {
          if (mounted) {
            setState(() {
              ...
              _controller.play();
              debugPrint('🎥 Video initialized and playing in LearnSubtraction');
            });
          }
        }).catchError((error) {
          debugPrint('❌ Video initialization error in LearnSubtraction: $error');
          if (mounted) {
            setState(() {
            ...
            Future.delayed(const Duration(seconds: 2), () {
              if (mounted && !_controllerInitialized) {
                debugPrint('🔄 Attempting to reinitialize video controller in LearnSubtraction (Retry $_videoRetryCount/$_maxVideoRetries)');
                _initializeVideoController();
              }
            });
          }
  ...
  void _resetVideoController() {
    if (_controllerInitialized) {
      ...
      _controllerInitialized = false;
      _videoRetryCount = 0;
      debugPrint('🔄 Resetting video controller in LearnSubtraction');
      _initializeVideoController();
    }
  }
  ...
  void _ensureVideoPlaying() {
    if (mounted && _controllerInitialized && !_controller.value.isPlaying) {
      _controller.play();
      debugPrint('▶️ Restarted video playback in LearnSubtraction');
    }
  }
  ...
  Future<bool> _onWillPop() async {
    ...
    return await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to leave this subtraction adventure? 🌟'),
        actions: [
          TextButton(
  ...
  void dispose() {
    ...
    if (_controllerInitialized) {
      _controller.dispose();
      debugPrint('🗑️ Disposed VideoPlayerController in LearnSubtraction');
    }
    super.dispose();
  }
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(
                                  "Hey, math champs! 🌟 Subtraction is like taking candies away from a jar—how many are left? Watch the fun video and start your subtraction adventure to become a superstar! 🚀",
                                  style: TextStyle(
                                    fontSize: 18,
                          ...
                          onTap: () {
                            ...
                            Navigator.push(
                              ...
                              PageRouteBuilder(
                                ...
                                pageBuilder: (context, animation, secondaryAnimation) => const ChooseSubtraction(),
