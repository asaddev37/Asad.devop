...
class _QuizPageState extends State<QuizPage> with TickerProviderStateMixin {
  ...
  String _mascotMessage = "Time to multiply, champ! 🚀";
  ...
  Future<void> _initializeAudioPlayer() async {
    try {
      ...
      debugPrint('🔊 AudioPlayer initialized in QuizPage');
    } catch (e) {
      debugPrint('❌ AudioPlayer initialization error in QuizPage: $e');
    }
  }
  ...
  void _generateQuiz() {
      ...
      for (var question in possibleQuestions.toList()) {
        ...
        if (currentWeight >= randomWeight && !_questionHistory.contains(question['question'])) {
          ...
          while (options.length < 4) {
            int deviation = question['correct'] <= 20 ? random.nextInt(3) + 1 : random.nextInt(5) + 2;
    ...
    if (_quizQuestions.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        showErrorOverlay(
          context: context,
          message: "No questions available! Try selecting more tables or a larger range! 🌟",
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      showErrorOverlay(
        context: context,
        message: "Your multiplication quiz adventure starts now! 🚀",
        onDismiss: () => _playSound('assets/sounds/cliick.wav'),
        tickerProvider: this,
      );
    });
  ...
  void dispose() {
    ...
    debugPrint('🗑️ Disposed AudioPlayer and ConfettiController in QuizPage');
    super.dispose();
  }
}